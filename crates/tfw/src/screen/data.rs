use crate::prelude::*;
use bevy::ecs::{component::ComponentId, schedule::ScheduleLabel, system::SystemId};
...
/// Call this when you want to switch screens but only have access to the
/// screen's [ComponentId]. Whenever possible, prefer to use [SwitchToScreen].
#[derive(Event, Debug, PartialEq, Eq, Clone, Deref)]
pub struct SwitchToScreenById(pub ComponentId);
...
pub struct ScreenRegistry(Vec<ScreenIndex>);

#[derive(Debug)]
pub struct ScreenIndex {
    pub name: String,
    pub id: ComponentId,
    pub trigger_load: SystemId,
}
